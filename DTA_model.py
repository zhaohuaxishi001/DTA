...
def inference(smi_tensor, pro_tensor, regularizer=None,
    ...
    with tf.compat.v1.variable_scope('drug_conv'):
        ...
        conv2_weights = tf.compat.v1.get_Variable(
            ...
            initializer=tf.truncated_normal_initializer(stddev=0.1))
        conv3_weights = tf.compat.v1.get_Variable(
        ...
        conv3_biases = tf.compat.v1.get_Variable(
    ...
    with tf.compat.v1.variable_scope('protein_conv'):
        ...
        variable_summaries(conv2_biases, 'b2')
        variable_summaries(conv3_biases, 'b3')
    with tf.compat.v1.variable_scope("attention_layer", reuse=tf.AUTO_REUSE):
    ...
    with tf.compat.v1.variable_scope('deep-fc-layer', reuse=tf.AUTO_REUSE):
    ...
    with tf.compat.v1.variable_scope('y-layer', reuse=tf.AUTO_REUSE):
